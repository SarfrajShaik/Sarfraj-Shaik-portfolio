...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    // Securely send an email to sarfrajshaik08@gmail.com
    ...
    if (!smtpUser || !smtpPass) {
      return res.status(400).json({ 
        error: 'Email delivery is not yet configured. Please configure your "SMTP_USER" and "SMTP_PASS" environment variables in Vercel to enable real delivery to sarfrajshaik08@gmail.com.' 
      });
    }
    ...
    const mailOptions = {
      ...
      html: `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e5e7eb; border-radius: 8px; background-color: #ffffff;">
          ...
          <p style="font-size: 11px; color: #9ca3af; text-align: center; margin: 0;">Sent securely via Vercel serverless integration.</p>
        </div>
      `,
    };
